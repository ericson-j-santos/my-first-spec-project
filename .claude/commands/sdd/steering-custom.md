...
2. **Check if template exists**:
   ...
   - **Exception — integration templates** (e.g. `jira.md`, `azure-devops.md`): display the setup guide embedded
     in the template's HTML comment block in chat, then write the file content verbatim without
     codebase analysis (external tool integrations have nothing to analyze in the repo)

3. **Analyze codebase** (JIT) for relevant patterns (skip for integration templates):
...
8. **jira.md** - Jira Cloud sync: auto-creates Epics, Stories, and Subtasks at each SDD phase
9. **azure-devops.md** - Azure DevOps Boards sync: auto-creates Epics, User Stories, and Tasks at each SDD phase

Load template when needed, customize for project (or write verbatim for integrations).
