...
1. **Load Context**:
   ...
   - Read `.sdd/specs/$ARGUMENTS/requirements.md` for project description
...
- **Non-numeric Requirement Headings**: If existing headings do not include a leading numeric ID, normalize them to numeric IDs and keep that mapping consistent (never mix numeric and alphabetic labels).

### Next Phase: Design Generation

**If Requirements Approved**:
- Review generated requirements at `.sdd/specs/$ARGUMENTS/requirements.md`
- **Optional Gap Analysis** (for existing codebases):
  - Run `/sdd:validate-gap $ARGUMENTS` to analyze implementation gap with current code
  - Recommended for brownfield projects; skip for greenfield
- Then `/sdd:spec-design $ARGUMENTS -y` to proceed to design phase

**If Modifications Needed**:
- Provide feedback and re-run `/sdd:spec-requirements $ARGUMENTS`
