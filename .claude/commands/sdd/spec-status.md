...
<instructions>
## Core Task
Generate status report for feature **$ARGUMENTS** showing progress across all phases.

## Execution Steps

### Step 1: Load Spec Context
- Read `.sdd/specs/$ARGUMENTS/spec.json` for metadata and phase status
- Read existing files: `requirements.md`, `design.md`, `tasks.md` (if they exist)
- Check `.sdd/specs/$ARGUMENTS/` directory for available files
...
**Spec Not Found**:
- **Message**: "No spec found for `$ARGUMENTS`. Check available specs in `.sdd/specs/`"
...
- Run with no argument to list all specs in `.sdd/specs/` with their status
