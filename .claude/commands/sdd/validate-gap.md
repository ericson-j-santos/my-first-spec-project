...
<instructions>
## Core Task
Analyze implementation gap for feature **$ARGUMENTS** based on approved requirements and existing codebase.
...
### Error Scenarios
- **Missing Requirements**: If requirements.md doesn't exist, stop with message: "Run `/sdd:spec-requirements $ARGUMENTS` first to generate requirements"
- **Requirements Not Approved**: If requirements not approved, warn user but proceed
...
- Run `/sdd:spec-design $ARGUMENTS` to create technical design document
- Or `/sdd:spec-design $ARGUMENTS -y` to auto-approve requirements and proceed directly
